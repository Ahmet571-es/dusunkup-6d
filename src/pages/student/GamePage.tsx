/**
 * Game Page — Routes to the correct game based on grade + gameId
 * Wraps each game in GameShell for HUD, session management, interventions
 */
...
const GAME_COMPONENTS: Record<string, Record<string, React.ComponentType<{ session: SessionManager; state: SessionState }>>> = {
  ...
  // sinif1-5 will be added in Faz 5-9
}
  ...
  return (
    <GameShell>
      {(session: SessionManager, state: SessionState) => {
        // If we have a real game component, render it
        if (GameComponent) {
          return <GameComponent session={session} state={state} />
        }
        ...
        return (
          <div className="flex flex-col items-center justify-center h-full p-6">
            <motion.div className="w-full max-w-md rounded-3xl p-8 text-center"
              ...
              <p className="text-white/50 text-sm mb-6">{gameDef.description}</p>
              <p className="text-white/20 text-xs">Bu oyun yakında eklenecek</p>
